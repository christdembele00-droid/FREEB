...
class CaptureManager(private val context: Context) {
    fun capturePhoto(imageCapture: ImageCapture, onResult: (Result<File>) -> Unit) {
        ...
        val file = File(
            dir,
            "FREEB_${SimpleDateFormat("yyyyMMdd_HHmmss_SSS", Locale.US).format(System.currentTimeMillis())}.jpg"
        )
    ...
    fun startVideo(
        ...
        onStarted: () -> Unit,
        onFinalized: (Result<File>) -> Unit
    ): Recording {
        val dir = File(context.cacheDir, "freeb/media").apply { mkdirs() }
        val file = File(
            dir,
            "FREEB_${SimpleDateFormat("yyyyMMdd_HHmmss_SSS", Locale.US).format(System.currentTimeMillis())}.mp4"
        )
